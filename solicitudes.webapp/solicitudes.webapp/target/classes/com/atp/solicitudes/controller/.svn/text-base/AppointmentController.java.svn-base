...
public class AppointmentController extends LocalBaseController
{
	...
	public String show(Map<String, Object> model, HttpServletRequest request)
	{
		...
		try
		{
			...
			{
				colStr.add(each.getDefinition());
			}
			model.put("appointments", colStr);
			...
			Transportista transportista = solicitud.getTransportistaId() == null ? null : managerN4.getTransportistaWithId(solicitud.getTransportistaId());
			OperadorTransportista operador = solicitud.getOperadorId() == null ? null : managerN4.getOperadorTransportistaWithGkey(solicitud.getOperadorId());

			model.put("cancelList", JSONUtils.getJSONFromObject(CancellationMotiveEnum.allValues));			

			model.put(Transportista.SLOT_NAME, transportista == null ? getValueOrNull(null) : JSONUtils.getJSONFromObject(transportista));
			model.put(OperadorTransportista.SLOT_NAME, operador == null ? getValueOrNull(null) : JSONUtils.getJSONFromObject(operador));
			...
			model.put("dayScheduled", solicitud.getDateScheduled() == null ? null : dayFormat.format(solicitud.getDateScheduled()));
			model.put("timeScheduled", solicitud.getDateScheduled() == null ? null : timeFormat.format(solicitud.getDateScheduled()));
			model.put("placas", solicitud.getPlacas());
...
//			manager.validateSolicitudCanBeModified(solicitud, scheduleDate);
			...
			solicitud.setPlacas(placas);
			solicitud.setDateScheduled(scheduleDate);
			solicitud.setTransportistaId(transportistaId);
			solicitud.setOperadorId(operadorId);
	...
	public void prepareImpoExpoAndDesistimiento(Solicitud solicitud, ArrayNode appointments) throws DomainModelException, Exception
	{
		...
		if(appointments.size()>1)
		{
			for (JsonNode each : appointments)
			{
					...
					{
						if (eachAppointmentNode.get("fk").asText().equals("FCL"))
						{
							if (eachAppointmentNode.get("contenido") == null || eachAppointmentNode.get("contenido").asText() != "null")
							{
